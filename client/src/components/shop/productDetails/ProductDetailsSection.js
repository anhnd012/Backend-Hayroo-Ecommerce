...
const ProductDetailsSection = (props) => {
  ...
  //console.log("sProduct" + sProduct.prod_name);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      //console.log(responseData.value[1].prod_image);
      setTimeout(() => {
        if (responseData.value) {
          ...
          setPimages(responseData.value.prod_image);
  ...
  const fetchCartProduct = async () => {
    try {
      ...
      if (responseData && responseData.value) {
        layoutDispatch({ type: "cartProduct", payload: responseData.value }); // Layout context Cartproduct fetch and dispatch
      }
    } catch (error) {
      console.log(error);
    }
        ...
        <div className="grid grid-cols-2 md:grid-cols-12">
          <div className="hidden md:block md:col-span-1 md:flex md:flex-col md:space-y-4 md:mr-2">
            <img
              ...
              src={`${apiURL}/uploads/products/${sProduct.prod_image}`}
              alt="pic"
            />
            <img
              onClick={(e) =>
          ...
          <div className="col-span-2 mt-8 md:mt-0 md:col-span-4 md:ml-6 lg:ml-12">
            <div className="flex flex-col leading-8">
              <div className="text-2xl tracking-wider">{sProduct.prod_name}</div>
              <div className="flex justify-between items-center">
                ...
                <span>
                  <svg
                    onClick={(e) => isWishReq(e, sProduct.prod_id, setWlist)}
                    className={`${
                      isWish(sProduct.prod_id, wList) && "hidden"
                  ...
                  <svg
                    onClick={(e) => unWishReq(e, sProduct.prod_id, setWlist)}
                    className={`${
                      !isWish(sProduct.prod_id, wList) && "hidden"
                    } w-5 h-5 md:w-6 md:h-6 cursor-pointer text-yellow-700`}
                    fill="currentColor"
            ...
            <div className="my-4 md:my-6 text-gray-600">
              {sProduct.prod_description}
            </div>
            <div className="my-4 md:my-6">
              {+quantitiy === +sProduct.quantity ? (
              ...
              <div
                className={`flex justify-between items-center px-4 py-2 border ${
                  +quantitiy === +sProduct.quantity && "border-red-500"
                }`}
              >
                <div
                  className={`${
                    quantitiy === sProduct.quantity && "text-red-500"
                  }`}
                >
                  Quantity
                </div>
              ...
              {sProduct.quantity !== 0 ? (
                ...
                <Fragment>
                  {layoutData.inCart !== null &&
                  layoutData.inCart.includes(sProduct.prod_id) === true ? (
                    <div
                      style={{ background: "#303031" }}
                      className={`px-4 py-2 text-white text-center cursor-not-allowed uppercase opacity-75`}
                    >
